...
/**
 * Définition de l'arbre. Lignes (row) réservées par branche pour un layout lisible :
 *   0 = feu, 1 = traque, (racines au centre entre les deux),
 *   3 = mobilité/défense, 4 = choc/contrôle. La row 2 sert de boomerang (dérivation).
 */
export const SPELL_TREE: TreeNode[] = [
  // --- Racines (col 0) ---
  { id: 'fireball', tier: 1, branch: 'fire', col: 0, row: 1, cost: 0, requires: [] },
  { id: 'dash', tier: 1, branch: 'move', col: 0, row: 3, cost: 0, requires: [] },

  // --- Branche Feu (offense directe) ---
  ...
  { id: 'homing', tier: 2, branch: 'hunt', col: 1, row: 1, cost: 1, requires: ['fireball'] },
  { id: 'boomerang', tier: 3, branch: 'hunt', col: 2, row: 2, cost: 1, requires: ['homing'] },
  { id: 'drain', tier: 3, branch: 'hunt', col: 2, row: 1, cost: 1, requires: ['homing'] },
  { id: 'link', tier: 4, branch: 'hunt', col: 3, row: 1, cost: 2, requires: ['drain'] },
  { id: 'grapple', tier: 5, branch: 'hunt', col: 4, row: 1, cost: 3, requires: ['link'] },

  // --- Branche Mobilité / défense ---
  { id: 'teleport', tier: 2, branch: 'move', col: 1, row: 3, cost: 1, requires: ['dash'] },
  { id: 'shield', tier: 3, branch: 'move', col: 2, row: 3, cost: 1, requires: ['teleport'] },
  { id: 'whirlwind', tier: 4, branch: 'move', col: 3, row: 3, cost: 2, requires: ['shield'] },

  // --- Branche Choc / contrôle ---
  { id: 'bolt', tier: 2, branch: 'shock', col: 1, row: 4, cost: 1, requires: ['dash'] },
  { id: 'gravity', tier: 3, branch: 'shock', col: 2, row: 4, cost: 1, requires: ['bolt'] },
  { id: 'scourge', tier: 4, branch: 'shock', col: 3, row: 4, cost: 2, requires: ['gravity'] },
  { id: 'laser', tier: 5, branch: 'shock', col: 4, row: 4, cost: 3, requires: ['scourge'] },
];
